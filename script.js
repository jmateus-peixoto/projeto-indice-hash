...
    }
    ...
    /**
     * MODIFICADO: Esta função foi alterada para criar uma estrutura de bucket mais complexa,
     * que separa os itens principais dos itens de overflow.
     */
...
        // Agora, cada posição é preenchida com um objeto que separa os itens principais dos de overflow.
        hashTable = Array.from({
            length: config.numBuckets
        }, () => ({
            items: [],      // Armazena itens até o limite (FR)
            overflow: []    // Armazena itens que excedem o limite
        }));
    }
...
    }
    ...
    /**
     * MODIFICADO: Esta função foi alterada para implementar a lógica de resolução de overflow.
     * Itens são adicionados à lista `overflow` quando a lista `items` atinge sua capacidade máxima.
     */
...
                const bucket = hashTable[bucketIndex]; // Acede ao bucket correto (que agora é um objeto).

                // CONTAGEM DE COLISÃO: Uma colisão ocorre se o bucket já contém qualquer item (principal ou overflow).
                if (bucket.items.length > 0 || bucket.overflow.length > 0) {
                    stats.collisions++;
                }

                const entry = { key: word, page: pageIndex };

                // LÓGICA DE RESOLUÇÃO DE OVERFLOW:
                // Se o bucket principal ainda não está cheio, adicione nele.
                if (bucket.items.length < config.bucketSize) {
                    bucket.items.push(entry);
                } 
                // Se o bucket principal está cheio, adicione na lista de overflow.
                else {
                    bucket.overflow.push(entry);
                    stats.overflows++; // Incrementa a estatística de overflow AQUI.
                }
            });
        });
    }
    ...
    /**
     * MODIFICADO: A busca foi atualizada para verificar primeiro a lista `items`
     * e, se necessário, a lista `overflow` do bucket.
     */
...
        const bucket = hashTable[bucketIndex]; // O bucket agora é um objeto {items: [], overflow: []}
        let cost = 1; // Simula 1 acesso a disco para ler a estrutura do bucket (principal + overflow).
        let found = false;
        let resultData = null;

        // 1. Procura primeiro nos itens principais do bucket.
        ...
        // 2. Se não encontrou, procura nos itens de overflow.
        if (!found) {
